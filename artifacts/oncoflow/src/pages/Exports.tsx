...
export default function Exports() {
  ...
  const handleSurvivalExport = async () => {
    toast({ title: t.exports.exportStarted, description: "Preparing survival CSV from AI predictions..." });
    try {
      const token = localStorage.getItem("oncoflow_token");
      const res = await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:4000/api"}/exports/survival-csv`,
      ...
      a.href = url;
      a.download = "survival_statistics.csv";
      a.click();
      URL.revokeObjectURL(url);
    } catch {
      toast({ title: "Export failed", description: "Could not generate survival report. Make sure predictions exist." });
    }
  };
        ...
        {exportOptions.map((opt) => (
          <Card key={opt.titleKey} className="shadow-sm hover:shadow-md transition-shadow border-gray-200">
            <CardContent className="p-6">
              ...
              <Button
                onClick={() => opt.isSurvival ? handleSurvivalExport() : handleExport(opt.titleKey)}
